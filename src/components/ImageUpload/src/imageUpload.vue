  ...
  <div class="component-upload-image">
      ...
      >
        <template #item="{ element }">
          <div class="items">
            ...
            <div class="img-mask">
              <a-space>
                <EyeOutlined @click="handlePreview(element)" class="mr-2"></EyeOutlined>
                <DeleteOutlined @click="removeFn(element)" ></DeleteOutlined>
    ...
    <div class="text-xs text-gray-400" v-if="showTip">
      <template v-if="fileSize">
        大小不超过
        <b class="text-red-400">{{ fileSize }}MB</b>
      </template>
  ...
  try {
    uploadApi({file}).then(async res => {
      if (res.status !== 200) {
        ...
        return createMessage.error(`上传失败`);
      } else {
